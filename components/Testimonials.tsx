    ...
    <section className="mx-auto w-full max-w-7xl px-5 py-16 md:px-10 md:py-24 lg:py-32" aria-labelledby="testimonials-title">
      ...
      <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-3">
        {testimonials.map((testimonial, index) => (
          <div key={testimonial} className="flex flex-col items-center p-8 text-center bg-gray-900/5 rounded-lg">
            <div className="mb-4 rounded-full bg-purple-100 p-2">
              {index % 2 === 0 ? 
                <Ghost className="h-12 w-12 text-purple-600" /> :
                <Skull className="h-12 w-12 text-purple-600" />
              }
            </div>
